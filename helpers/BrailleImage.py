...
from helpers.BrailleConverter import *
...
class BrailleImage(object): 
    def __init__(self, image):
        ...
        self.converted = self.convert_braille_image(image_path=image)
    ...
    def convert_braille_image(self, image_path, scale=100):
        original_img, resized_img = read_and_resize_img(image_path, scale)
        thresh_1 = preprocess_img(resized_img)
        edged = detect_edge(thresh_1)
        cnts = find_contour(edged)
        circle_img, contour_img, _ = draw_contour(cnts, resized_img)
        
        return circle_img

    def bound_box(self,left,right,top,bottom,color= (0,0,0), size=1):
